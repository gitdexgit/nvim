...
local function is_normal_win(win)
    ...
    return ok and (bt == "" or bt == "acwrite")
end

local function push_win(win)
    if not is_normal_win(win) then return end
    for i, id in ipairs(win_stack) do
        if id == win then table.remove(win_stack, i) break end
    end
    table.insert(win_stack, 1, win)
    for i = #win_stack, 1, -1 do
        if not vim.api.nvim_win_is_valid(win_stack[i]) then
...
vim.api.nvim_create_autocmd("VimEnter", {
    callback = function()
        push_win(vim.api.nvim_get_current_win())
    end,
})
...
vim.keymap.set("n", "<leader>Y", function()
    ...
    -- 2. Use "+y$ to yank to system clipboard from cursor to end of line
    -- We use y$ because it is the "true" command for what you want
    vim.cmd('normal! "+y$')
...
vim.keymap.set("i", "<C-o><M-!>", function()
    run_shell(false)
end)
